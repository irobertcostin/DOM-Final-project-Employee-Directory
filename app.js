...
population.addEventListener("click",(b)=>{
    ...
    if(obj.classList.contains("modal-delete")){
        ...
        remove(staff,x.email);
        if(currentPage==0){
            populeazaPagina(staff,1)
        }else{
            populeazaPagina(staff,currentPage);
        }
...
arrows.addEventListener("click",(f)=>{
    ...
    if(obj.id=="left"){
        ...
        let nexObj = next(staff,email.toLowerCase());
        // console.log(employeeCard)
        // console.log(nexObj)
...
// dupa remove, ramane empty spot in grid
// remove-ul se face doar din grid, nu si din staff
// dupa delete, elementul reapare
// de ce nu pot folosi mereu ultimul state al arrayului? 
// 